...
## Phase 9 — Implemented (2026-03-28)
### Dashboard (`src/pages/Dashboard.js`)
- Fetches `id, idea, geography, created_at` from Supabase `reports` table filtered by `user.id`
- Loading state: 3 animated skeleton cards (`skeleton-pulse` CSS animation)
- Empty state: 📋 icon + "No reports yet." + "Validate an Idea →" button that navigates to `/` and scrolls to form
- Report cards: idea text (truncated, 1 line), date (right), geography orange pill, "View Report →" link; hover brightens border; click → `/report/:id`

### ReportViewer (`src/pages/ReportViewer.js`)
- Fetches full report by `id` from Supabase; RLS handles access (private = owner only, public = anyone)
- **Sticky top bar**: glassmorphic, "← My Reports" link + "Share" + "Export PDF" buttons
- **Share**: marks `is_public=true` in Supabase, copies URL to clipboard, shows "Link copied!" for 2s
- **Export PDF**: `window.print()` — print dialog allows Save as PDF natively
- **Report header**: "VALIDATION REPORT" eyebrow, idea as H1, geography pill + date + "Generated by Scrutin"
- **ReactMarkdown** (v8, CRA-compatible): custom `components` for h1/h2/h3/p/ul/ol/li/strong/hr — all styled with design system variables
- Loading skeleton + "Report not found." error state
- Route: `/report/:id` (no ProtectedRoute — RLS handles access)
- Print styles added to `index.css`: hides nav/top-bar/buttons during print

## Prioritized Backlog

### P0 — Features
- Make.com webhook → write report_content back to Supabase `reports` table
- Add `user_id` to reports when user is authenticated
